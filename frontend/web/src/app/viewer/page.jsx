...
export default function ViewerModePage() {
  ...
  const shareCatchup = async () => {
    ...
    const streamerName =
      ...
      || "this stream";
    const recapSnippet = String(catchupRecap.recap || "").slice(0, 120);
    const shareText = `Just joined ${streamerName}'s stream and Zumi told me exactly what I missed in 5 seconds.\n\n"${recapSnippet}..."\n\nWatch with context: ${window.location.origin}/viewer?s=${encodeURIComponent(String(activeStreamerId || ""))}`;
    try {
      if (navigator.share) {
  ...
  const fetchStreamers = async (queryValue = "") => {
    try {
      ...
      const params = new URLSearchParams();
      params.set("limit", q ? "30" : "20");
      if (q) {
        params.set("q", q);
        params.set("include_external", "true");
      }
      const res = await apiFetch(`/auth/streamers?${params.toString()}`);
      if (!res.ok) return;
      const data = await res.json().catch(() => ({}));
      setAvailableStreamers(Array.isArray(data?.streamers) ? data.streamers : []);
      setStreamerDiagnostics(data?.diagnostics || null);
    } catch (error) {
              ...
              filteredStreamers.map((streamer) => {
                ...
                return (
                  <article key={streamer.id} className="viewer-streamer-card">
                    ...
                    {streamer.source === "external" ? (
                      <button type="button" className="viewer-secondary-button viewer-watch-button" onClick={() => openExternalStreamer(streamer)}>Watch</button>
                    ) : (
                      <button
                        type="button"
